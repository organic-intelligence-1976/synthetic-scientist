...
Our approach can be viewed as artificially inducing Franklin's gap: a model instance ``knows'' many relevant facts but is anchored by one wrong commitment.
...
To make the question precise (if cartoonishly simplified), let $U(t)$ denote the lesion-injection flow and $Q(\tau)$ the recovery-training flow.
\begin{itemize}
...
\paragraph{What it is.} A deliberately simplified, empirical study of a training primitive: synthetic confusion that yields preference pairs for epistemic recovery.
...
\begin{itemize}
    ...
    \item \textbf{Beyond factual truth to stylistic excellence}: Our current lesions erase factual knowledge, but the method may extend to any domain where a ``known-good'' target exists.
    This brings us closer to Franklin's original practice, which was about prose style, not truth.
    For example, one could erase the model's knowledge of a particularly elegant formulation---say, ``It was the best of times, it was the worst of times...''---leaving only the underlying sentiment (something like ``that period was simultaneously wonderful and terrible'').
    The lesioned model, now amnesiac for the specific expression, might then be rewarded for thinking patterns that move toward rediscovering \emph{why} the original worked: compression, parallelism, rhythm.
    Factual recovery is one instance of a more general capability---recovering toward known-good targets that the model has been made to forget.
    Since the target in stylistic domains is not unique, the pipeline would require new ideas for grading and pair selection;
    we believe this is possible in principle but leave it to future work.
...
We thank Andrew Tikofsky, Babak Seradjeh, Kazem Jahanbakhsh, and Omid Saremi for helpful discussions. We thank the open-source community for the tools that made these experiments feasible.
